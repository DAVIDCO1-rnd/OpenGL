function [bids E Ner] = find_delaunay_boundary03_hole_fig(X,Fd)
%inputs:
...
%xlim([-3,3]); ylim([-3,+4]);%for C-shape
%xlim([-30,-9.5]); ylim([-32,+32]);%for A-shape
%xlim([-33,13]); ylim([-33,+43]);%for M-shape
%xlim([-32,36]); ylim([-32,+42]);%for MA-shape
%xlim([19,56]); ylim([-43,+43]);%for Rect-hole-shape

[bids E Ne] = find_delaunay_boundary03_fig1(X,Fd);
nP = size(E,1);
%nBnd = size(bndry,2);

V = E; %V(i,j) = 1: edge not visited yet; 0: no edge; 2: edge already visited
Ner = Ne;
Er = E;
for i = 1:nP
    for j = i+1:nP
...
for i = 1:nP
    for j = 1:nP
        if (i < j && V(i,j) == 1 && (Ne(i,j) == 2 || Ne(j,i) == 2))% && ~V(i,j)) % check only the upper triangle; V(i,j) = 1 indicates this is not-visted edge and
            %Ne(i,j) == 2 means this is an inside edge
...
%make removed edges white 
for ii = 1:size(Xr,1)
    plot([Xr(ii,1) Xr(ii,3)], [Xr(ii,2) Xr(ii,4)],'-w'); hold on;
end  
%show boundary edges
for i = 1:nP
    for j = 1:nP
...
while size(Q,1)>0
    ...
    while(1)
        ...
        for m = 1:size(vs,2)
            ...
            if k ~= pv           && sum(chk) == 0 && (Ner(cv,k) == 1 || Ner(k,cv) == 1)%k is not pv, all of whose possible neigbours are not already in Vc and E(j,k) is used only once
